...
# Tmux window switching functions
# Switch to lazygit window or create it if it doesn't exist
function tmux_goto_lazygit() {
    local window_index=$(tmux list-windows -F "#{window_index}:#{window_name}" | grep ":lazygit$" | head -1 | cut -d: -f1)
    if [[ -n "$window_index" ]]; then
        tmux select-window -t "$window_index"
    else
        tmux new-window -n lazygit "lazygit -ucf ~/.config/lazygit/config.yml"
    fi
}

# Switch to opencode window or create it if it doesn't exist
function tmux_goto_opencode() {
    local window_index=$(tmux list-windows -F "#{window_index}:#{window_name}" | grep ":opencode$" | head -1 | cut -d: -f1)
    if [[ -n "$window_index" ]]; then
        tmux select-window -t "$window_index"
    else
        tmux new-window -n opencode
        tmux send-keys -t opencode "opencode -c" Enter
    fi
}

# Switch to nvim window or create it if it doesn't exist
function tmux_goto_nvim() {
    local window_index=$(tmux list-windows -F "#{window_index}:#{window_name}" | grep ":nvim$" | head -1 | cut -d: -f1)
    if [[ -n "$window_index" ]]; then
        tmux select-window -t "$window_index"
    else
...
# Switch to zsh window or create it if it doesn't exist
function tmux_goto_zsh() {
    local window_index=$(tmux list-windows -F "#{window_index}:#{window_name}" | grep ":zsh$" | head -1 | cut -d: -f1)
    if [[ -n "$window_index" ]]; then
        tmux select-window -t "$window_index"
    else
...
# Switch to yazi window or create it if it doesn't exist
function tmux_goto_yazi() {
    local window_index=$(tmux list-windows -F "#{window_index}:#{window_name}" | grep ":yazi$" | head -1 | cut -d: -f1)
    if [[ -n "$window_index" ]]; then
        tmux select-window -t "$window_index"
    else
